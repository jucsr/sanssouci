...
public class AddNewFaceMill extends AddNewFaceMillFrame implements ActionListener, ItemListener
{
	...
	public AddNewFaceMill(Frame owner)
	{
		super(owner);
	...
	private void ok() 
	{
		int id = this.projectTools.faceMillTable.getRowCount() + 1;
		...
		DefaultTableModel modelo = (DefaultTableModel)this.projectTools.faceMillTable.getModel();
		this.projectTools.faceMillTable.setModel(modelo);
		modelo.addRow(linha);
		this.dispose();
	}
}
