...
//	public ArrayList<Path> mergulho;
	...
	private ArrayList<Path> calcularMergulho()
	{
		...
		double course; //distancia que devera ser percorrida pela ferramenta do ponto da ferramenta at� ponto inicial
		...
		else if (a == 'r') 
		{
			...
			/* CASO EM QUE A FERRAMENTA PARA EM UM TRECHO LINEAR*/
				...
				else
				{
					...
					double alfa = Math.atan2((pathCircular.getInitialPoint().y - centralPoint.y), (pathCircular.getInitialPoint().x - centralPoint.x));// angulo entre ponto central e inicial, em rela��o a abscissa
...
//			long times = Math.round(Math.floor(course/width)); //numero de vezes que completa o width. (floor -> arredonda para baixo. retorna double) (round -> ARREDONDA pra inteiro)
			...
			double espaco=0;//quantidade que a ferramenta anda at� o ultimo ponto onde troca de caminho
			int voltas=0;
			int cont = 0;
			System.out.println("curso total da ferramenta: " + course);
			if (course<width)
			{
				System.out.println("Error! 'Width' given is too big.");
/***/				return null; //recurso para quebrar condicao if 
			}

//ADICIONANDO PATHS DO WIDTH
			while (width >= t) // vai encontrar at� onde o width ir� percorrer os caminhos
			{
				if(paths.get(cont).getClass().equals(LinearPath.class)) //se o caminho for reto
				{
					LinearPath pTmp = (LinearPath)paths.get(cont);							
				...
				{	
					espaco=t; //espaco armazena o caminho percorrido efetivo (quando o caminho eh adicionado ao widthPath) 
					if (cont == (paths.size()-1)) //se completou uma volta. ESPERA-SE QUE SEJA SEMPRE FALSO. IMPLEMENTADO POR SEGURANCA
					{
						cont = 0;
						voltas ++;
					}
			...
			double sobra = width - espaco; // QUANTO FALTA PERCORRER. width menos espaco percorrido ateh ultimo caminho
			
//ADICIONANDO ULTIMO PATH DO WIDTH
...
//caso CIRCULAR - ultima parte widthPath
			else if (paths.get(cont).getClass().equals(CircularPath.class))
			{
				...
				double alfa = Math.atan2((circular.getInitialPoint().y - pontoC.y), (circular.getInitialPoint().x - pontoC.x));// angulo entre centro e inicial, em rela��o a abscissa
...
/***trajeto esta sendo escrito de tras pra frente. 
 * converter! */
			
				int aux = 0;
				double sobra1=0; 
				boolean flag = true; //true = avan�ando / false = voltando
				...
				while(course>=0) //SE AINDA HA CAMINHO A PERCORRER
				{
					...
					if (widthPath.get(aux).getClass()==LinearPath.class) //SE EH LINEAR
					{
						...
						novaAlturaZ = alturaZ + (distTemp*Math.tan(angle));
						...
						if (course>=0) // SE AINDA HA CAMINHO A PERCORRER, DEPOIS DA SUBTRACAO, DEVE ADICIONAR O TRAJETO
						{
							if(flag==true)//indo
							{
								tempInicial = new Point3d (widthPath.get(aux).getInitialPoint().x,widthPath.get(aux).getInitialPoint().y, alturaZ);
							 	tempFinal = new Point3d (widthPath.get(aux).getFinalPoint().x,widthPath.get(aux).getFinalPoint().y, novaAlturaZ);
							}
							else		//voltando
							{
								tempInicial = new Point3d (widthPath.get(aux).getFinalPoint().x,widthPath.get(aux).getFinalPoint().y, alturaZ);
								tempFinal = new Point3d (widthPath.get(aux).getInitialPoint().x,widthPath.get(aux).getInitialPoint().y, novaAlturaZ);
							}
							trajeto.add(new LinearPath (tempInicial,tempFinal));
							alturaZ=novaAlturaZ;
						}
					}
					//CIRCULAR
					else if(widthPath.get(aux).getClass()==CircularPath.class)
					{
						...
						Point3d pontoI = new Point3d(cTmp.getInitialPoint().x, cTmp.getInitialPoint().y, alturaZ);
						alturaZ = alturaZ + (distTemp*Math.tan(angle));
						...
						if (course>=0) //aqui eu ia criar uma condicao para verificar se a altura respeita o retractPlane, mas achei desnecess�rio pois neste caso a verificacao do 'course' tambem falha
					...
					if (course>=0)//SE ADICIONOU O TRAJETO E VAI CONTINUAR NO WHILE
					{
						...
						if (aux == (widthPath.size()-2) && (flag==true)) //QUANDO PASSOU O PENULTIMO PATH, na ida, O ULTIMO EH ESSA EXCESSAO 
						{
							...
							if (course>=0)
							{
								novaAlturaZ = alturaZ + (distTemp*Math.tan(angle));
								tempInicial = new Point3d (widthPath.get(aux).getFinalPoint().x,widthPath.get(aux).getFinalPoint().y, alturaZ);
							 	tempFinal = new Point3d (widthPoint.x, widthPoint.y, novaAlturaZ);
								trajeto.add(new LinearPath (tempInicial,tempFinal));
								sobra1=course;
								alturaZ=novaAlturaZ;
							}
							...
							{
								novaAlturaZ = alturaZ + (distTemp*Math.tan(angle));
								tempInicial = new Point3d (widthPoint.x, widthPoint.y, alturaZ);
								tempFinal = new Point3d (widthPath.get(aux).getFinalPoint().x,widthPath.get(aux).getFinalPoint().y, novaAlturaZ);
								trajeto.add(new LinearPath (tempInicial,tempFinal));
								sobra1=course;
								alturaZ=novaAlturaZ;
...
//				double ultimo = high/Math.tan(angle) - sobra1; //course ja foi reescrito, mas era high/tangente
				...
				{
					Point3d ponto = new Point3d(trajeto.get(aux).getInitialPoint().x,trajeto.get(aux).getInitialPoint().y,alturaZ);
					toolPoint = new Point3d (trajeto.get(aux).getInitialPoint().x + sobra1*Math.cos(ang), trajeto.get(aux).getInitialPoint().y + sobra1*Math.sin(ang), retractPlane);
					trajeto.add(new LinearPath(ponto,toolPoint));
				}
				else //se estava voltando
				{
					Point3d ponto = new Point3d(trajeto.get(aux).getFinalPoint().x,trajeto.get(aux).getFinalPoint().y,alturaZ);
					toolPoint = new Point3d (trajeto.get(aux).getFinalPoint().x - sobra1*Math.cos(ang), trajeto.get(aux).getFinalPoint().y - sobra1*Math.sin(ang), retractPlane);
					trajeto.add(new LinearPath(ponto,toolPoint));
				}
		}
