...
public class MapeadoraFuroBaseEsferica {

	private Projeto projeto;
	private Bloco bloco;
	private Face faceTmp;
	private FuroBaseEsferica furoTmp;
	...
	public MapeadoraFuroBaseEsferica(Projeto projeto, Face face,
		...
		this.mapearFuroBaseEsferica();

	}

	private void mapearFuroBaseEsferica() {

		Workingstep wsTmp;
		Workingstep wsPrecedenteTmp = null;
		
		double retractPlane = 5;
		double pontaEsferica = furoTmp.getFloorRadius();
		...
		if (furoTmp.isAcabamento()) {
			...
			if (furoTmp.getDiametro() >= 1.0) { // Verifica se é necessário
				...
				wsTmp.setId("SphericalBottomHole_RGH");
			...
			if (hasEqualsDiameter(furoTmp.getDiametro(), this.reamers, bloco
				...
				if (reamer != null) {
					...
					wsTmp.setId("SphericalBottomHole_FNS");
					wsTmp.setWorkingstepPrecedente(wsPrecedenteTmp);
					wsPrecedenteTmp = wsTmp;
			...
			BallEndMill ballEndMill = chooseBallEndMill(bloco.getMaterial(),
					ballEndMills, furoTmp, pontaEsferica);
	...
	private CenterDrill chooseCenterDrill(Material material,
			ArrayList<CenterDrill> centerDrills, FuroBaseEsferica furoTmp,
...
//					"Erro", JOptionPane.ERROR_MESSAGE);
			...
			.showMessageDialog(
					...
					"\tFeature: Furo Base Esférica \n" +
					"\tNome: " + furoTmp.getNome() +"\n" +
					"\tProfundidade: " + furoTmp.getProfundidade()+" mm"+"\n" +
					"\tDiametro: " + furoTmp.getDiametro()+" mm"+"\n" +
					"\tMaterial Bloco: " + material.getName()+"\n" +
					"__________________________________________________________"+"\n"+
			...
			throw new NullPointerException("Nenhum Center Drill selecionado");
	...
	private TwistDrill chooseTwistDrill(Material material,
			ArrayList<TwistDrill> twistDrills, FuroBaseEsferica furoTmp,
...
//					"Erro", JOptionPane.ERROR_MESSAGE);
	...
	private FaceMill chooseFaceMill(Material material,
			ArrayList<FaceMill> faceMills, FuroBaseEsferica furoTmp,
			double limite_desbaste) {
...
//			.showMessageDialog(
//					null,
//					"Não é possível fazer o acabamento deste Furo Base Esferica com os Reamers atuais, \n " +
...
//					JOptionPane.WARNING_MESSAGE);
	...
	private BoringTool chooseBoringTool(Material material,
			ArrayList<BoringTool> boringTools, FuroBaseEsferica furoTmp) {
...
//					"Erro", JOptionPane.ERROR_MESSAGE);
	...
	private BallEndMill chooseBallEndMill(Material material,
			ArrayList<BallEndMill> ballEndMills, FuroBaseEsferica furoTmp,
