...
import br.UFSC.GRIMA.cad.visual.FuroBaseConicaFrame;
...
public class CriarFuroBaseConica extends FuroBaseConicaFrame implements ActionListener
{
	Face face = null;
	public JanelaPrincipal parent;
	public CriarFuroBaseConica(JanelaPrincipal parent, Face face) {
		super(parent);
		// this.setModal(true);
		this.face = face;
		this.parent = parent;
...
//				  "Erro na dimens�o X", JOptionPane.CANCEL_OPTION); }
		...
		if (ok)
		{
			angulo = ((Double) this.angulo.getValue()).doubleValue();
		}
		...
		if (ok) {
			...
			FuroBaseConica furoBaseConica = new FuroBaseConica();
			furoBaseConica.setPosicao(X, Y, Z);
			furoBaseConica.setProfundidade(profundidade);
			furoBaseConica.setRaio(raioFuro);
			furoBaseConica.setTipAngle(angulo);
			furoBaseConica.setNome(this.textField9.getText());
			furoBaseConica.setTolerancia(tolerancia * 0.001);
			furoBaseConica.setRugosidade(rugosidade * 0.001);
			
			if(tolerancia<=Feature.LIMITE_RUGOSIDADE && rugosidade<=Feature.LIMITE_TOLERANCIA){
				furoBaseConica.setAcabamento(true);
			}
			
			
			if (this.face.validarFeature(furoBaseConica))
			{		
				...
				position.setName(furoBaseConica.getNome() + " placement");
				furoBaseConica.setPosition(position);	
