...
public class CriarFuroBasePlana extends FuroFrame implements ActionListener {
	
	private static final long serialVersionUID = -3392739602905872067L;
	public Face face = null;
	public JanelaPrincipal parent;
	public static int holeIndex = 1;
	
	public CriarFuroBasePlana(JanelaPrincipal parent, Face face) {
		super(parent);
		// this.setModal(true);
		this.face = face;
		this.parent = parent;
		// this.parent.setEnabled(false);
//		this.spinner1.setModel(new SpinnerNumberModel(face.getProfundidadeMaxima(), null, null, 1.0));
...
//		System.out.println(this.getLocation().toString());
	...
	private void ok() {
		...
		if (ok) {
			try {
				posicaoX = ((Double)this.spinner2.getValue()).doubleValue();
		...
		if (ok) {
			try {
				posicaoY = ((Double)this.spinner4.getValue()).doubleValue();
				if (posicaoY > 0) {
					ok = true;
				} else {
					ok = false;
		...
		if (ok) {
			try {
				raioFuro = ((Double)this.spinner3.getValue()).doubleValue();
		...
		if (ok) {
			...
			FuroBasePlana novoFuro = new FuroBasePlana();
			...
			novoFuro.setPassante(this.checkBox1.isSelected());
