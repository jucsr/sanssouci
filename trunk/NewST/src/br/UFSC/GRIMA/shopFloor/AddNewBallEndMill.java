...
public class AddNewBallEndMill extends AddNewBallEndMillFrame implements ActionListener, ItemListener
{
	...
	public AddNewBallEndMill(Frame owner) 
	{
		super(owner);
	...
	private void ok()
	{
		int id = this.projectTools.ballEndMillTable.getRowCount() + 1;
		...
		Object[] linha = {id, nome, diametro, dm, cuttingEdge, profundidade,offSetLength, edgeRadius, edgeCenterVertical, hand, material, this.materialClass};
		DefaultTableModel modelo = (DefaultTableModel)this.projectTools.ballEndMillTable.getModel();
		this.projectTools.ballEndMillTable.setModel(modelo);
		modelo.addRow(linha);
		this.dispose();
	}
}
