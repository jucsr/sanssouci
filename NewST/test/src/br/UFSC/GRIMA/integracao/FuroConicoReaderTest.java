...
public class FuroConicoReaderTest {
	...
	public void getFuroTeste(){
		try {
			FuroConico furo = FuroConicoReader.getFuro(eRound_hole);
			...
			Assert.assertEquals(20.0, prof, 0);
			
			double tolerancia = furo.getTolerancia();
			Assert.assertEquals(0.05, tolerancia, 0);
			
			double raio1 = furo.getRaio1();
			Assert.assertEquals(6.0, raio1, 0);
